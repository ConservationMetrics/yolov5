...
def detect(opt, save_img=False):
    ...
    if webcam:
        ...
        if len(source_files) > 0:
            dataset = LoadFiles(source_files, img_size=imgsz, stride=stride)
        else:
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i in range(n_y):
            for j in range(n_x):
                ...
                # make crop
                cropped_img = torch.from_numpy(
                    img[
                        :,
                        offset_y_new : min(offset_y_new + tile_y, img_height),
                        offset_x_new : min(offset_x_new + tile_x, img_width),
                    ]
                ).to(device)

                #       img = torch.from_numpy(img).to(device)
                cropped_img = (
                    cropped_img.half() if half else cropped_img.float()
                )  # uint8 to fp16/32
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--object-size-px", type=int, default=30, help="object size (pixels)"
    )
    ...
    parser.add_argument(
        "--save-img", action="store_true", help="save results to image/movie file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-xxyy",
        action="store_false",
        help="true to save xywh and false to save xxyy format",
    )
    parser.add_argument(
    ...
    if testing:
        ...
        opt = Namespace(
            ...
            save_conf=True,
            save_txt=True,
            source="D:/croz_20201129_v1_tiled-3-4.tif",
            source_files=[],
            update=False,
            save_img=True,
            view_img=False,
            weights=[
                r"D:/CM,Inc/Dropbox (CMI)/CMI_Team/Analysis/2019/PointBlue_Penguins_2019/models/exp42/weights/best.pt"
            ],
            save_xxyy=False,
        )
